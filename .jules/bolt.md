...
## 2024-05-26 - [Canvas Animation & DOM Optimization]
**Learning:** In high-frequency animation loops (60fps), operations like 'ctx.createLinearGradient', DOM lookups ('document.getElementById'), and array re-allocations ('.slice().map()') create significant CPU overhead and GC pressure. Caching the gradient (updating only on resize), caching DOM elements, and using direct loops for path generation can reduce per-frame execution time by over 65%. Additionally, replacing 'innerText' with 'textContent' avoids unnecessary forced reflows during these high-frequency updates.
**Action:** Always cache expensive graphical assets and DOM references outside of high-frequency loops (animations, mouse moves). Replace modern array methods with simple for-loops in performance-critical paths to minimize memory churn. Use 'textContent' for frequent text updates.
