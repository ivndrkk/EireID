...
## 2026-04-27 - [Canvas & DOM Animation Optimization]
**Learning:** For high-frequency animations (60fps), eliminating object/array allocations () inside the render loop significantly reduces Garbage Collection pressure. Caching DOM elements and  objects, and using  instead of  (which triggers forced reflows), results in a major performance boost (~74% in this case).
**Action:** Always move object creation and DOM selection outside of  loops. Use  for coordinate storage to minimize memory overhead. Favor  for updating text values in real-time.

## 2026-04-27 - [Canvas & DOM Animation Optimization]
**Learning:** For high-frequency animations (60fps), eliminating object/array allocations (`.slice().map()`) inside the render loop significantly reduces Garbage Collection pressure. Caching DOM elements and `CanvasGradient` objects, and using `textContent` instead of `innerText` (which triggers forced reflows), results in a major performance boost (~74% in this case).
**Action:** Always move object creation and DOM selection outside of `requestAnimationFrame` loops. Use `Float32Array` for coordinate storage to minimize memory overhead. Favor `textContent` for updating text values in real-time.
