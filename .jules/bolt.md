...
## 2026-06-10 - [Canvas Animation Loop Optimization]
**Learning:** In 60fps canvas animations, per-frame memory allocations (e.g., from `.slice().map()`) and redundant path calculations are primary performance killers due to garbage collection pressure and CPU overhead. Utilizing the `Path2D` API to define a path once and reuse it for multiple operations (stroke/fill) combined with manual `for` loops significantly reduces frame-time jitter. Additionally, caching canvas gradients and only re-initializing them on container resize prevents expensive context state resets on every frame.
**Action:** Eliminate all array/object allocations inside high-frequency `requestAnimationFrame` loops. Use `Path2D` for reusable vector paths and cache all static drawing resources (gradients, patterns) outside the loop.
