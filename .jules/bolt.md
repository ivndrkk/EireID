...
## 2026-06-10 - [High-Frequency Animation Loop Optimization]
**Learning:** High-frequency animation loops (e.g., 60fps Canvas drawing) are extremely sensitive to DOM access and object allocations. Repeatedly calling `document.getElementById` or using array methods that create new arrays (like `slice().map()`) inside these loops triggers frequent garbage collection and layout thrashing. Caching DOM references, using typed arrays (`Float32Array`), and replacing functional array chains with direct `for` loops can improve execution efficiency by ~80%+.
**Action:** For any animation or high-frequency event handler (scroll, resize, mousemove), strictly cache all DOM references and pre-calculate as many values as possible. Avoid `.map()`, `.filter()`, or `.slice()` inside the loop; use direct index-based loops and typed arrays for numerical data. Use `textContent` instead of `innerText` for value updates.
