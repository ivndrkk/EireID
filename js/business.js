...
(function () {
    ...
    function initHeroGraph() {
        ...
        window.addEventListener('resize', () => {
            ...
            cachedGradient = null; // Re-init gradient with new dimensions
            drawGraph(1);
        });
        ...
        let cachedValueSpan = null;

        /**
         * Optimizations:
         * 1. Cached CanvasGradient (re-init on resize)
         * 2. Cached DOM element #hero-graph-value
         * 3. Replaced innerText with textContent to avoid forced reflow
         * 4. Eliminated array allocations (.slice.map) in animation loop
         */
        function drawGraph(progress) {
            ...
            if (!cachedValueSpan) {
                cachedValueSpan = document.getElementById('hero-graph-value');
            }
            if (cachedValueSpan) {
                const maxVal = 24.5;
                const currentVal = (maxVal * progress).toFixed(1);
                cachedValueSpan.textContent = `${currentVal}M+`;
            }
        }
