...
(function () {
    ...
    function initHeroGraph() {
        ...
        // Optimization: Cache DOM element and gradient to avoid per-frame overhead
        ...
        window.addEventListener('resize', () => {
            width = canvas.width = canvas.parentElement.clientWidth;
            height = canvas.height = canvas.parentElement.clientHeight * 0.7;
            initGradient(); // Re-init gradient on resize
            drawGraph(1);
        });
        ...
        function drawGraph(progress) {
            ...
            // Optimization: Replace .slice().map() with a direct loop to avoid object allocations
